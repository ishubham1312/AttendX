...
class _TodayBanner extends StatelessWidget {
  final AttendanceEntry? entry;
  const _TodayBanner({required this.entry});

  @override
  Widget build(BuildContext context) {
    final text = entry == null
        ? 'Not marked yet · tap the circle above'
        : "Today marked as ${entry!.displayLabel}";
      ...
      child: Row(
        children: [
          AnimatedContainer(
            ...
            height: 36,
            decoration: BoxDecoration(
              color: color.withValues(alpha: entry != null ? 0.12 : 0.06),
              borderRadius: BorderRadius.circular(10),
            ),
            child: Icon(
              entry == null ? Icons.info_outline : Icons.event_available,
              color: color,
              size: 18,
            ),
          ),
